...
public class ValidationServiceTest {
	...
	public void testValidationForAll() throws Exception {
		...
		assertEquals(260, run.getCompletedAssertions().size());
		assertEquals(0, run.getSkippedAssertions().size());
		assertEquals(12, run.getFailedAssertions().size());
		...
		final List<String> failedMessages = Arrays.asList(
				...
				"The attribute value of 3311482005 |Has presentation strength denominator value concrete (attribute)| must conform to the MRCM attribute domain range int(#30..#40)",
				"The attribute value of 3311482005 |Has presentation strength denominator value concrete (attribute)| must conform to the MRCM attribute domain range str(\"test\")",
	...
	private void runValidationForAttribute(String attributeId, List<ValidationType> types) throws ReleaseImportException, IOException, ServiceException {
		...
		domainFound.getAttributes().stream().filter(attribute -> attribute.getAttributeId().equals(attributeId))
				.forEach(domainToValidate::addAttribute);
	...
	public void testValidationForConcreteValue() throws Exception {
		...
		final List<String> failureAssertionMessages =
				Arrays.asList("The attribute value of 3311482005 |Has presentation strength denominator value concrete (attribute)| " +
				"must conform to the MRCM attribute domain range dec(#10..#20)",
				"The attribute value of 3311482005 |Has presentation strength denominator value concrete (attribute)| " +
				"must conform to the MRCM attribute domain range int(#30..#40)",
				"The attribute value of 3311482005 |Has presentation strength denominator value concrete (attribute)| " +
				"must conform to the MRCM attribute domain range str(\"test\")");
		assertEquals(3, run.getFailedAssertions().size());
		run.getFailedAssertions().forEach(failed -> {
			assertTrue(failureAssertionMessages.contains(failed.getAssertionText()));
			assertEquals(375745003L, failed.getCurrentViolatedConceptIds().get(0).longValue());
		});
	}
	...
	public void testValidationForConcreteDataType() throws Exception {
		...
		final List<String> failedAssertionMessages =
				Arrays.asList("Concrete value #1 found but no concrete data type is defined in the MRCM range constraint",
						"Concrete value #100.0 found but no concrete data type is defined in the MRCM range constraint",
						"Concrete value #500 found but no concrete data type is defined in the MRCM range constraint");
		final Set<Assertion> assertions = run.getFailedAssertions();
		...
		run.getFailedAssertions().forEach(failed -> {
			...
			assertTrue(failed.getCurrentViolatedConceptIds().contains(375745003L));
		});
	}
}
